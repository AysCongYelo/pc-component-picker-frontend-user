...
class _CartScreenState extends State<CartScreen> {
 ...
 Future<void> _loadCart() async {
  ...
  Future<void> _removeSelectedItems() async {
    ...
    try {
      for (final id in idsToRemove) {
        await _api.delete("/cart/deleteRow/$id");
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Failed to remove selected items: $e"),
            backgroundColor: Colors.redAccent,
          ),
        );
      }
  ...
  // ---------------- CART ITEM TILE (NEW LAYOUT + BUILD BUNDLE IMAGE) ----------------
            ...
            children: [
              ...
              ClipRRect(
                ...
                child: Container(
                  ...
                  child: (() {
                    // BUILD BUNDLE → ALWAYS SHOW HARDCODED IMAGE
                    if (category == "build_bundle") {
                      return Image.asset(
                        "assets/build_bundle.png",
                        fit: BoxFit.cover,
                      );
                    }

                    // NORMAL COMPONENT IMAGE
